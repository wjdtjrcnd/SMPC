...
#       # Matches self.u_dv in structure but timesteps range from -1, ..., N-1.
    ...
    def _add_cost(self):
        cost = 0
        for i in range(self.N):
            cost += RefTrajGenerator._quad_form(self.z_dv[i+1, :].T - self.wp_ref[i,:].T, ca.diag([1.,2*(i+1),1.,1.])@self.Q) # tracking cost

        for i in range(self.N-1 ):
            cost += RefTrajGenerator._quad_form(self.u_dv[i+1,:].T-self.u_dv[i,:].T, self.R)  # input cost

        cost += 1*(ca.sum1(self.sl_df_dv) + ca.sum1(self.sl_acc_dv))  # slack cost
...
class SMPC_MMPreds():
    ...
    def _return_policy_class(self, i, N_TV, t_bar):
        ...
        if t_bar == 0 or t_bar==self.N-1:
            ...
            h=[[self.opti[i].variable(2,1) for n in range(1+(-1+self.N_modes**N_TV)*int(t>=t_bar))] for t in range(self.N)]
            # M=[[[self.opti[i].variable(2, 4) for n in range(1+(-1+self.N_modes)*int(t>=t_bar))] for j in range(t)] for t in range(self.N)]
            M=[[[ca.DM(2, 4) for n in range(1+(-1+self.N_modes**N_TV)*int(t>=t_bar))] for j in range(t)] for t in range(self.N)]
            K=[[[self.opti[i].variable(2,2) for k in range(N_TV)] for n in range(1+(-1+self.N_modes)*int(t>=t_bar))] for t in range(1,self.N)]

            M_stack=[ca.vertcat(*[ca.horzcat(*[M[t][j][m*int(t>=t_bar)] for j in range(t)], ca.DM(2,4*(self.N-t))) for t in range(self.N)]) for m in range(self.N_modes**N_TV)]
            h_stack=[ca.vertcat(*[h[t][m*int(t>=t_bar)] for t in range(self.N)]) for m in range(self.N_modes**N_TV)]
    ...
    def _add_constraints_and_cost(self, i, N_TV, t_bar):
        ...
        for j in range(1+(-1+self.N_modes**N_TV)*(t_bar>0)):
            ...
            for t in range(1,self.N):
                ...
                for k in range(N_TV):
                    p=mode(j,k)
                    z=-2*(oa_ref[k]-ca.vertcat(self.x_tv_ref[i][k][p][t], self.y_tv_ref[i][k][p][t])).T@self.Q_tv[i][k][p][t-1]@(ca.horzcat(B_block[t*4:t*4+2,:]@M[j]+mmr_std[j]*E_block[t*4:t*4+2,:], *[B_block[t*4:t*4+2,:]@K[mode(j,l)][l]@F_block[l][mode(j,l)][:2*self.N,:]-int(l==k)*mmr_std[j]*F_block[k][p][t*2:(t+1)*2,:] for l in range(N_TV)]))
